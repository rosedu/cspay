...
/*
 * se teseteaza conversia inaltimii liniilor
...
main(void)
{
	...
	float rlsz;
	int i;
	doc = spreadconv_new_spreadconv_data("Data", 51, 1);
	#define START	5
	#define I	(i - START)
	for (i = START; i < 50; ++ i) {
		cn = malloc(512);
		...
		cs->name = cn;
		cs->type = LSC_STYLE_ROW;
		sz = malloc(512);
		rlsz = (float) i / 10;
		snprintf(sz, 512, "%2.1fcm", rlsz);
		cs->size = sz;
		doc->cells[I][0].text = strdup(sz);
		cs_[I] = spreadconv_add_unique_rc_style(cs, doc);
	}
	for (i = START; i < 50; ++ i) {
		spreadconv_set_row_style(I, cs_[I], doc);
	}
