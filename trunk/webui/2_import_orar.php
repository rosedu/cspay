...
$query = "UPDATE `asocieri` SET `link_admin`='-1'";
$result = mysql_query($query);
add($mesaj,'Tabela asociatie a fost stearsa.<br>');
$query = "DELETE FROM `utilizatori` WHERE `tip_cont`='".PROFESOR."'";
$result = mysql_query($query);
...
$query = "INSERT INTO `ore` (".$NumColumn.") VALUES";
...
$query = "SELECT * FROM `ore` WHERE `link_univ`='".$_SESSION['univ_id']."' GROUP BY `materie`";
$result = mysql_query($query);
$nr = mysql_num_rows($result);

if($nr)
{
$query = "INSERT INTO `utilizatori` (`materie`,`utilizator`,`parola`,`tip_cont`,`link_univ`) VALUES";
for($i=0;$i<$nr;$i++)
{
