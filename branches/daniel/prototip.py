...
def output_table(input, excluded):
	...
	for curs in courses:
		i=insert_course(S, i, curs, input.get_year(), input.get_month(), excluded, course_count)
	labs=input.get_labs()
	for lab in labs:
		i=insert_lab(S, i, lab, input.get_year(), input.get_month(), excluded, apps_count)
...
def write_totals(S, i, C, A):
	...
	for j in range(0, 4):
		qq.write(ws, i+3+j, 3, qq.formula(form_sum(course_count[internal[j]],C_col)),qq.boxed(1))
		qq.write(ws, i+3+j, 5, qq.formula(form_sum(apps_count[internal[j]],A_col)),qq.boxed(1))
...
if __name__ == "__main__":
        ...
        lab1={ 'poz': 'as', 'no':87, 'fac':'A & C', 'day':2,
               'name':'Utiliz Sist Operare','duration':2,'group':'324CA','hour':12}
        lab2={ 'poz': 'sl', 'no':67, 'fac':'A & C', 'day':4,
               'name':'Utiliz Sist Operare','duration':3,'group':'321CA','hour':10}
        lab3={ 'poz': 'as', 'no':57, 'fac':'A & C', 'day':1,
               'name':'Utiliz Sist Operare','duration':2,'group':'322CA','hour':11}
