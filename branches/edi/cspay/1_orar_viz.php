...
//	add($content,'<div align="center"><input type="submit" name="salveaza" value="Salveaza"></div>');
	...
	add($content,display_thead_orar_prof2());//scrie capul de tabel

	for($i=$start;$i<$stop;$i++)//pentru  fiecare intrare
	{
		$output = display_result_prof($result_orar,$i);//scrie linia respectiva sub forma de tabel
		add($content,$output);
	}
	add($content,'</table>');
	...
	{
	add($content,"<br>Momentan nu exista informatii disponibile.");	
	}
