...
/*
function display_facultate($univ_id,$value,$index_class)
{
	global $class_select;
	...
	add($output,'<select class='.$class_select[$index_class%2].'>');
...
function display_select($name,$value,$hidden_id,$index_class,$vector)
{
	...
	$output = '';
	add($output,'<select  class="'.$class_select[$index_class%2].'" '.' name="'.$name.'"
				 onchange="change_hidden(\''.$hidden_id.'\',\'1\');">');
...
function display_materie($univ_id)
{
	$output = '';
	$query = "SELECT `nume_scurt` FROM `facultate` WHERE `link_univ`='".$univ_id."'";
	$result = mysql_query($query);
...
function display_result_secr($result,$index)
{
	...
	list($orar_id, $facultate, $tip_curs1, $materie, $forma, $cod, $an, $serie, 
...
$valori = array(5,10,15,20,30,50,"tot");

$select2 = '';
add($select2,'<form action="" method="post" >
			 <select name="nr_pe_pag" onchange="this.form.submit();">');
...
add($content,'<div align="center"><table border="0" cellpadding="0" cellspacing="0"><tr valign="bottom"><td valign="top">');
add($content,$link1.'&nbsp;&nbsp;&nbsp;</td><td>'.
			 $select.
			 '</td><td valign="top">&nbsp;&nbsp;&nbsp;'.$link2.'&nbsp;&nbsp;&nbsp;'.
			 '</td><td>'.$select2.'</tr></table></div>');	
return $content;
}
