...
if(isset($_POST['facultate_adauga']))//formular de adaugare facultate
{
	...
	//textul interogarii
	$query = "INSERT INTO `facultate` (`link_univ`,`nume`,`nume_scurt`,`decan`) VALUES ('".addslashes($_SESSION['univ_id'])."',
...
if(isset($_POST['catedra_adauga']))//formularul de adaugare catedra
{
	...
	//textul interogarii
	$query = "INSERT INTO `catedra` (`link_fac`,`nume`,`sef`) VALUES ('".$link_fac."','".$nume."','".$sef."');";
	if(mysql_query($query))//daca interogarea este corecta
	{
...
if(isset($_POST['catedra_modifica']))//formularul de modificare catedra
{
	...
	$query = "UPDATE `catedra` SET `link_fac`='".$link_fac."',`nume`='".$nume."',`sef`='".$sef."' 
...
if(isset($_POST['facultate_modifica']))//formularul de modificare facultate
{
	...
	$query = "UPDATE `facultate` SET `nume`='".$nume."',`nume_scurt`='".$nume_scurt."',`decan`='".$decan."'
...
if(isset($_GET['catedra_sterge']))//parametru pentru stergerea catedrei
{
	...
	$query = "DELETE FROM `catedra` WHERE `cat_id`='".addslashes($_GET['catedra_sterge'])."'";
...
{
	$query = "DELETE FROM `facultate` WHERE `fac_id`='".addslashes($_GET['facultate_sterge'])."'";
	if(mysql_query($query))
	{
...
$query = "SELECT * FROM `facultate`";//selectez facultatile
...
for($i=0;$i<$nr;$i++)
	{
		$nume_fac = mysql_result($res_fac,$i,'nume');
		$id_fac = mysql_result($res_fac,$i,'fac_id');
		add($content,'<option value="'.$id_fac.'">'.htmlspecialchars($nume_fac).'</option>');
	}
...
for($i=0;$i<$nr;$i++)//scriu fiecare intrare din tabela `facultate`
{
	...
	$nume = mysql_result($res_fac,$i,'nume');
	$nume_scurt = mysql_result($res_fac,$i,'nume_scurt');
	$decan = mysql_result($res_fac,$i,'decan');
	
	//scriu datele in tabel pe un rand nou
	add($content,'<tr class="'.$class[$i%2].'">
				  <td>'.($i+1).'</td>
				  <td>'.htmlspecialchars($nume).'</td>
				  <td>'.htmlspecialchars($nume_scurt).'</td>
				  <td>'.htmlspecialchars($decan).'</td>
	...
	$query = "SELECT * FROM `catedra` WHERE `link_fac` = '".$id_fac."'";
	...
	for($j=0;$j<$nr_cat;$j++)
	    {
		$nume = mysql_result($res_cat,$j,'nume');
		$sef = mysql_result($res_cat,$j,'sef');
		$id_catedra = mysql_result($res_cat,$j,'cat_id');
		
		add($content,'<tr class="'.$class[$j%2].'"><td width="33%">'.$nume.'</td><td width="33%">'.$sef
...
if($flag_form == CATEDRA_MODIFICA)
{
		...
		$nume = mysql_result($res_cat,0,'nume');
		$sef = mysql_result($res_cat,0,'sef');
		...
		$nume_fac_select = mysql_result($res_fac,0,'nume');
...
for($i=0;$i<$nr;$i++)
	{
		$nume_fac = mysql_result($res_fac,$i,'nume');
		$id_fac = mysql_result($res_fac,$i,'fac_id');
		add($content,'<option value="'.$id_fac.'"'.( ($nume_fac_select=$nume_fac)?' selected':'') .'>'.addslashes($nume_fac).'</option>');
	}
							
add($content,'</select>
				</td>
				<td>
				Nume <input type="text" name="nume" value="'.htmlspecialchars($nume).'">
				</td>
				<td>
				Sef <input type="text" name="sef" value="'.htmlspecialchars($sef).'">
				</td>
...
if($flag_form == FACULTATE_MODIFICA)
{
		...
		$nume = mysql_result($res_fac,0,'nume');
		$nume_scurt = mysql_result($res_fac,0,'nume_scurt');
		$decan = mysql_result($res_fac,0,'decan');
...
add($content,'<form action="0_facultate.php" method="post"> 
			...
			<table class="formular">
				...
				<td>
				Nume scurt <input type="text" name="nume_scurt" value="'.htmlspecialchars($nume_scurt).'">
				</td>
				<td>
				Decan <input type="text" name="decan" value="'.htmlspecialchars($decan).'">
				</td>
